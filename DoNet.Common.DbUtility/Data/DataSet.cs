...
namespace DoNet.Common.DbUtility.Data
{
    /// <summary>
    /// 查询结果集
    /// </summary>
                ...
                foreach (var item in r.ItemArray)
                {
                    if (!string.IsNullOrWhiteSpace(item.Value))
                    {
                        try
                        {
                            ...
                            {
                                row[item.ColumnName] = Convert.FromBase64String(item.Value);
                            }
                            else
                            {
    ...
    public class DataRow
    {
        ...
        private void FromRow(System.Data.DataRow dr,DataColumn[] columns)
        {
            ...
            for (var i = 0; i < ItemArray.Length; i++)
            {
                ...
                if (obj != null)
                {
                    if (columns[i].DataType.Equals("System.Byte[]", StringComparison.OrdinalIgnoreCase))
                    {
